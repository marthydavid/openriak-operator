...
var _ = Describe("RiakUser Controller", func() {
	...
	Context("CRD schema validation", func() {
		It("rejects creation when certificateRef is omitted", func() {
			userName := "user-missing-certref"
			u := &riakv1.RiakUser{
				ObjectMeta: metav1.ObjectMeta{Name: userName, Namespace: ns},
				Spec: riakv1.RiakUserSpec{
					ClusterName: "some-cluster",
			...
			Expect(err.Error()).To(ContainSubstring("certificateRef"))
		})
	})

	Context("certificate creation fails", func() {
		newFailCertCreateClient := func() client.Client {
			wc, err := client.NewWithWatch(cfg, client.Options{Scheme: k8sClient.Scheme()})
			Expect(err).NotTo(HaveOccurred())
			return interceptor.NewClient(wc, interceptor.Funcs{
				Create: func(ctx context.Context, c client.WithWatch, obj client.Object, opts ...client.CreateOption) error {
					if obj.GetObjectKind().GroupVersionKind().Kind == certManagerKind {
						return fmt.Errorf("certificate create injected failure")
					}
		...
		It("sets phase to Failed when creating the cert-manager Certificate fails", func() {
			clusterName := "user-certcreate-fail-c"
			userName := "user-certcreate-fail"
			...
			r := &RiakUserReconciler{Client: newFailCertCreateClient(), Scheme: k8sClient.Scheme()}
			req := reconcile.Request{NamespacedName: types.NamespacedName{Name: userName, Namespace: ns}}
			// First reconcile: adds finalizer, sets status Creating
			_, err := r.Reconcile(ctx, req)
			Expect(err).NotTo(HaveOccurred())
			// Second reconcile: cluster is Ready, Certificate creation fails → Failed
			_, err = r.Reconcile(ctx, req)
			Expect(err).NotTo(HaveOccurred())

			got := &riakv1.RiakUser{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: userName, Namespace: ns}, got)).To(Succeed())
			Expect(got.Status.Phase).To(Equal(riakv1.UserPhaseFailed))
			Expect(got.Status.Error).To(ContainSubstring("failed to reconcile certificate"))
		})
	})
